...
#		return np.sum((hist1-hist2)**2)
	...
	def candidates_from_colorhist(self, hist, features, cosine_similarity=False):
		result = []
		names = []
		for key in features.keys():
			if cosine_similarity:
				d = (cosine(hist[:,0], features[key][:,0]) + cosine(hist[:,1], features[key][:,1]) + cosine(hist[:,2], features[key][:,2])) * 1/3
				# print("Cosine distance: " + str(d))
			else:
